...
class GamePanel extends JPanel {
	...
	public GamePanel() {
		...
		ImageIcon img = new ImageIcon("images/chicken.jpg"); //이미지파일 부르기
		targetLabel = new JLabel(img);
		targetLabel.setSize(img.getIconWidth(),img.getIconWidth());

		bulletLabel.setSize(10,10); //총알크기정하기
		bulletLabel.setOpaque(true);
		bulletLabel.setBackground(Color.RED); //색깔 빨간색으로 지정
		add(baseLabel);
		add(targetLabel);
		add(bulletLabel);
	}
